...
class corte_caja_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(corte_caja_report, self).__init__(cr, uid, name, context=context)
        self.total = 0.0
        self.localcontext.update({
            'time': time,
            'strip_name': self._strip_name,
            'getpayments': self._get_payments,
            'getdevols': self._get_devols,
    ...
    def _get_devols(self, objects):
        result = 0.0
        for obj in objects:
            for line in obj.lines:
                if (line.price_subtotal<0):
                    result = result + line.price_subtotal
        return result
...
report_sxw.report_sxw('report.pos.session.corte_caja',
                                            'pos.session',
                                            'addons/corte_caja/report/corte_caja.rml',
                                            parser=corte_caja_report,
                                            header=False)
