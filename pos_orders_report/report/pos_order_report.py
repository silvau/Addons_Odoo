...
class pos_order_report(osv.osv):
    ...
    _description = "Point of Sale Orders Resume"
    _auto = False
    _columns = {
        ...
        'pos_reference': fields.char('Reference', size=128, readonly=True),
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'pos_order_report')
        cr.execute("""
            create or replace view pos_order_report as (
                select
                    min(l.id) as id,
                    count(*) as nbr,
                    ...
                    s.state as state,
                    s.pos_reference as pos_reference,
                    s.name as name,
                ...
                group by
                    ...
                    s.user_id,s.shop_id,s.company_id,s.sale_journal,l.product_id,s.create_date,s.pos_reference,s.name,s.session_id
                having
