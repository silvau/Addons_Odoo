...
class pos_details_products(report_sxw.rml_parse):
    ...
    def _pos_sales_products_details(self, form):
        data = self._pos_sales_details(form)
        if data:
            data=sorted(data, key=lambda tup: tup['code'])
            totals = defaultdict(int)
            for c in data:
                totals[c['code'],c['name'],c['uom']] += c['qty']
            tuples = totals.items()
    ...
    def __init__(self, cr, uid, name, context):
        super(pos_details_products, self).__init__(cr, uid, name, context=context)
        self.total = 0.0
        self.qty = 0.0
        ...
        self.localcontext.update({
            ...
            'pos_sales_products_details':self._pos_sales_products_details,
...
report_sxw.report_sxw('report.pos.details_products', 'pos.order', 'pos_product_report/report/pos_details_products.rml', parser=pos_details_products, header='internal')
